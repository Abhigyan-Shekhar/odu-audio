...
class SpeakerAttribution:
    ...
    def is_patient(self, threshold: float = 0.5) -> bool:
        ...
        """
        return (
            self.status == AttributionStatus.PATIENT and
    ...
    def is_reliable(self, min_confidence: float = 0.7) -> bool:
        ...
        """
        return (
            self.status not in {AttributionStatus.UNKNOWN, AttributionStatus.LOW_CONFIDENCE} and
            not self.overlap and
            self.patient_probability >= min_confidence
        )
...
class EnrollmentConfig:
    ...
    def validate(self):
        ...
        if self.method == AttributionMethod.CHANNEL_ASSIGNMENT:
            assert self.channel_index is not None, "channel_index required for channel_assignment"
